...
async fn test_token_vault() {
    println!(">>>>>>>>>> SUCCESS: test c98_vault <<<<<<<<<<<<<<<");
...
async fn test_nft_vault() {
    println!(">>>>>>>>>> SUCCESS: test NFT vault <<<<<<<<<<<<<<<");
    let mut context = c98_vault_program_test().start_with_context().await;
    let payer_wallet = get_default_wallet().unwrap();
...
#[tokio::test]
async fn test_claim_before_schedule() {
...
#[tokio::test]
async fn user_not_in_schedule_claim() {
    println!(">>>>>>>>>> FAIL: test user not in schedule claim  <<<<<<<<<<<<<<<");
    let mut context = c98_vault_program_test().start_with_context().await;
    let payer_wallet = get_default_wallet().unwrap();
    airdrop(&mut context, &payer_wallet.pubkey(), 10_000_000_000).await.unwrap();
    let feed_path: Vec<u8> = (0..10).map(|_| { rand::random::<u8>() }).collect(); // create random path
    ...
    let hacker_c98_token_account = create_associated_token_account(&mut context, &hacker.pubkey(), &c98_mint.pubkey()).await.unwrap();
    let hacker_cusd_token_account = create_associated_token_account(&mut context, &hacker.pubkey(), &cusd_mint.pubkey()).await.unwrap();
    ...
    mint_tokens(&mut context, &c98_mint.pubkey(), &vault_c98_token_account, 1_000_000_000_000, &payer_wallet.pubkey(), Some(&payer_wallet)).await.unwrap();
    mint_tokens(&mut context, &cusd_mint.pubkey(), &vault_cusd_token_account, 1_000_000_000_000, &payer_wallet.pubkey(), Some(&payer_wallet)).await.unwrap();
    mint_tokens(&mut context, &c98_mint.pubkey(), &hacker_c98_token_account, 1_000_000_000_000, &payer_wallet.pubkey(), Some(&payer_wallet)).await.unwrap();
    mint_tokens(&mut context, &cusd_mint.pubkey(), &hacker_cusd_token_account, 1_000_000_000_000, &payer_wallet.pubkey(), Some(&payer_wallet)).await.unwrap();
    ...
    let result = process_transaction_with_error(&mut context, &Vec::from([redeem_token_data]), &Vec::from([&hacker])).await;
    match result {
...
#[tokio::test]
async fn test_mismatch_claim() {
    println!(">>>>>>>>>> FAIL: user 1 claim user2's token <<<<<<<<<<<<<<<");
    let mut context = c98_vault_program_test().start_with_context().await;
    let payer_wallet = get_default_wallet().unwrap();
    airdrop(&mut context, &payer_wallet.pubkey(), 10_000_000_000)
        .await
