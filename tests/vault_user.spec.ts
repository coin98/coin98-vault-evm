...
describe('vault administrative task tests', function() {
  ...
  it('redeem token successful', async function() {
    ...
    await sut.connect(owner).createEvent(salt, currentTimestamp, whitelistRoot, c98Token.address, ZERO_ADDRESS);
    ...
    await sut.connect(account1).redeem(salt, 0, account1Address, 1000000, 0, proofs);
  ...
  it('redeem legacy token successful', async function() {
    ...
    const whitelistTree = createWhitelistTree(whitelists);
    const whitelistRoot = '0x' + whitelistTree.root().hash.toString('hex');
    await sut.connect(owner).createEvent(salt, currentTimestamp, whitelistRoot, usdtToken.address, ZERO_ADDRESS);
    ...
    await sut.connect(account2).redeem(salt, 1, account2Address, 1000000, 0, proofs);
  ...
  it('other user claim', async function() {
    ...
    await sut.connect(account2).redeem(salt, 0, account1Address, 1000000, 0, proofs);
    const balanceAfter = await c98Token.balanceOf(account1Address);
  ...
  it('wrong index', async function() {
    ...
    await hheh.time.increaseTo(currentTimestamp);
    await expect(sut.connect(account3).redeem(salt, 1, account3Address, 1000000, 0, proofs))
  ...
  it('wrong recipient', async function() {
    ...
    await hheh.time.increaseTo(currentTimestamp);
    await expect(sut.connect(account3).redeem(salt, 2, account2Address, 1000000, 0, proofs))
      .to.be.revertedWith('C98Vault: Invalid proof');
  });
  ...
  it('wrong amount', async function() {
    ...
    await hheh.time.increaseTo(currentTimestamp);
    await expect(sut.connect(account3).redeem(salt, 2, account3Address, 2000000, 0, proofs))
      .to.be.revertedWith('C98Vault: Invalid proof');
  });
  ...
  it('wrong proofs', async function() {
    ...
    await hheh.time.increaseTo(currentTimestamp);
    await expect(sut.connect(account3).redeem(salt, 2, account3Address, 1000000, 0, proofs))
      .to.be.revertedWith('C98Vault: Invalid proof');
  });
  ...
  it('early claim', async function() {
    ...
    const whitelistTree = createWhitelistTree(whitelists);
    const whitelistRoot = '0x' + whitelistTree.root().hash.toString('hex');
    await sut.connect(owner).createEvent(salt, currentTimestamp + 100, whitelistRoot, c98Token.address, ZERO_ADDRESS);
  ...
  it('user cannot redeem twice', async function() {
    ...
    const balanceAfter = await c98Token.balanceOf(account1Address);
    expect((balanceAfter.sub(balanceBefore)).toNumber()).eq(1000000);
    await expect(sut.connect(account1).redeem(salt, 0, account1Address, 1000000, 0, proofs))
